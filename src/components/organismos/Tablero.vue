<template>
    <section class="quoridor__board">
        <section v-for="(row, i) in board" :key="i" class="quoridor__row">
            <board-cell v-for="(cell, j) in row" :key="j" :x="j" :y="i" :isPlayer1="isPlayer1(i, j)"
                :isPlayer2="isPlayer2(i, j)" :isObstacle="isObstacle(i, j)" @click="handleClick(i, j)" />
        </section>
    </section>
</template>

<script>
import BoardCell from '../moleculas/Casilla.vue';
